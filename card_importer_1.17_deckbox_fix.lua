...
--[[Deck spawning]]
function stripDeckboxHTML(html)
  html = html:gsub('\n', ''):gsub('\r\n', '')
  _, _, body = html:find('<body>(.+)</body>')
  body = body:gsub('<br/>', '\n'):gsub('</?p>', '\n\n')
...
function spawnDeck(wr,qTbl)
  text = wr.text
  if wr.url:find('deckbox')then
    text = stripDeckboxHTML(wr.text)
  end
  if text:find('!DOCTYPE')then
    ...
    local sideboard=''
    local deck,list={},text:gsub('\n%S*Sideboard(.*)',function(a)sideboard=a return ''end)
    local maybeboard=sideboard:match('\n%S*Maybeboard(.*)')
--[[if qTbl.mode=='Sideboard'then list=text:match('Sideboard(.*)')end
